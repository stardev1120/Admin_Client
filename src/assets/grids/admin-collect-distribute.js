var Datatable_Distribute_AJAX_DEMO = function () {
    var datatable;
    var baseUrl;
    ...
    var demo = function (filter) {
        datatable = $('.m_datatable_distributes').mDatatable({
            data: {
                type: 'remote',
                source: {
                    read: {
                        method: 'GET',
                        url: baseUrl + '/admin-collect-distribute',
                        headers: headers,
                        map: function (raw) {
                            var dataSet = raw;
            ...
            columns: [
                ...
                },
                {
                    ...
                    //width: 70
                    template: function (row) {
                        return row.amount + ' ' + row.Loan.currency;
                    }
                },
                {
                    field: 'Loan',
